...
class NativeMailerHandlerTest extends TestCase
{
    ...
    public function testSend()
    {
        ...
        $this->assertEmpty($GLOBALS['email']);

        // non-empty batch
        $mailer->handle($this->getRecord(Logger::ERROR, "Foo\nBar\r\n\r\nBaz"));
        $this->assertNotEmpty($GLOBALS['email']);
        $this->assertInternalType('array', $GLOBALS['email']);
        $this->assertArrayHasKey('0', $GLOBALS['email']);
        $params = $GLOBALS['email'][0];
